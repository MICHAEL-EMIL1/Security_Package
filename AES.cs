...
namespace SecurityLibrary.AES
{
    ...
    {
        private string[,] DoMatrix(string text)
        {
        ...
        //mix columns for encrypt
        string[,] MixColumn = new string[,]{
            {"02","03", "01", "01"},
            {"01","02", "03", "01"},
            {"01","01", "02", "03"},
            {"03","01", "01", "02"}
        ...
        //mix columns for decrypt
        string[,] Inverse_MixColumn = new string[,]{
            {"0E","0B", "0D", "09"},
            {"09","0E", "0B", "0D"},
            {"0D","09", "0E", "0B"},
            {"0B","0D", "09", "0E"}
        ...
        //functions for both encrypt and decrypt
        // mix columns used for encrypt
        private string[,] MixColumns(string[,] matrix)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        StringBuilder binary1 = new StringBuilder(HexatoBinary(matrix[k, j]));
                        string res = "";
                        string _1B = HexatoBinary("1B");

                        if (MixColumn[i, k].Equals("02"))
                        {
                            if (binary1[0] == '1')
                            {
                                SL(binary1);
                        ...
                        else if (MixColumn[i, k].Equals("01"))
                        {
                            res = binary1.ToString();
                        }
                        else if (MixColumn[i, k].Equals("03"))
                        {
                            res = binary1.ToString();
                            if (binary1[0] == '1')
                            {
                        ...
                        if (k == 3)
                        {
                            mixed[i, j] = BinarytoHexa(mixed[i, j]).PadLeft(2, '0').ToUpper();
                        }
                    }
                }
            }
        ...
        //inverse mix columns used for decrypt
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        StringBuilder binary1 = new StringBuilder(HexatoBinary(matrix[k, j]));
                        string res = "";
                        if (Inverse_MixColumn[i, k].Equals("09", StringComparison.OrdinalIgnoreCase))
                        {
                            res = _09(binary1.ToString());
                        }
                        else if (Inverse_MixColumn[i, k].Equals("0B", StringComparison.OrdinalIgnoreCase))
                        {
                            res = _0B(binary1.ToString());
                        }
                        else if (Inverse_MixColumn[i, k].Equals("0D", StringComparison.OrdinalIgnoreCase))
                        {
                            res = _0D(binary1.ToString());
                        }
                        else if (Inverse_MixColumn[i, k].Equals("0E", StringComparison.OrdinalIgnoreCase))
                        {
        ...
        // shift rows right
        private string[,] SRR(string[,] matrix)
        {
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < i; j++)
        ...
        // used in key scheadule for the position of rows 
        ...
        //key schedualing 
        private string[,] KeyScheduling(string[,] key)
        {
        ...
        //some functions used in shifting 
        // hexa to binary
        static string HexatoBinary(string x)
        {
            x = Convert.ToString(Convert.ToInt32(x.ToString(), 16), 2).PadLeft(8, '0');
            return x;
        }
        // binary to hexa
        static string BinarytoHexa(string x)
        {
            x = Convert.ToString(Convert.ToInt32(x.ToString(), 2), 16);
            return x;
        }
        //shift left
        static string SL(StringBuilder binary1)
        {
            for (int i = 0; i < 8; i++)
            {
                if (i == 7)
        ...
        static String Shift_1B(string bin)//shift left and xor with 1B
        {
            ...
            {
                return BinXOR((bin.Remove(0, 1) + "0"), HexatoBinary("1B"));
            }
        }
        //used when last bit is 9
        ...
        {
            string[,] Cipher = DoMatrix(cipherText);
            string[,] KEY = DoMatrix(key);
            string[,] keyschedule = KeyScheduling(KEY);

            Cipher = XOR(Cipher, RoundKey(keyschedule, 10));

            for (int i = 10; i > 0; i--)
            {
                Cipher = SRR(Cipher);
                Cipher = SubBytes(Cipher, inverse_sbox);
                Cipher = AddRoundKry(Cipher, RoundKey(keyschedule, i - 1));
                if (i != 1)
        ...
        public override string Encrypt(string plainText, string key)
        {
            ...
            string[,] Plain = DoMatrix(plainText);
            string[,] KEY = DoMatrix(key);
            string[,] keyschedule = KeyScheduling(KEY);

            Plain = XOR(Plain, RoundKey(keyschedule, 0));

            for (int i = 0; i < 10; i++)
            {
                Plain = SubBytes(Plain, sbox);
                Plain = SSL(Plain);
                if (i != 9)
                    Plain = MixColumns(Plain);
                Plain = AddRoundKry(Plain, RoundKey(keyschedule, i + 1));
            }
